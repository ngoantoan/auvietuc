...
//**********************************************/
    ...
    function getActivedParentList($lang){
        $sql = "SELECT title,id FROM product_category WHERE status =1 and ISNULL(parent_id) AND lang='".$lang."' ORDER BY `id`";
        return $this->db->Execute($sql);
    }
    ...
    function getChildrensByNode($lang,$parentId){
        $sql = "SELECT id,parent_id,title,description,status
         FROM product_category WHERE status =1 and parent_id = $parentId AND lang= $lang ORDER BY `id`";
        return $this->db->Execute($sql);
    }
    ...
    function getSqlListByCateId($language_id=1, $category_id ){
        $sql=" SELECT a.id,ac.title as category,a.title,a.image,a.order,view_number,a.status,a.news_date".
            " FROM service_resort a ,product_category ac".
    ...
    function getSqlAdmin($title=NULL,$username=NULL, $type,$category_id=NULL ){
        $sql=" SELECT a.id,ac.title as title_cate,a.title,a.image ,a.description,a.detail,a.tag,a.type,a.status,a.created_at  ".
            " FROM service_resort as a ,product_category ac ".
...
//        $this->db->debug=true;
    ...
    function getNewsByCategoryId($category_id = null,$limit=null,$news_type=null, $is_visit=null ){
        ...
        if(!empty($category_id)){
            ...
            $s .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id = $category_id AND status = 1) ";
            $s .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id IN(SELECT id FROM product_category WHERE parent_id = $category_id AND status = 1))) ";
        }
        if(!empty($news_type))
    ...
    function getNewsByCategoryIdByAdmin($category_id = null,$limit=null,$news_type=null, $is_visit=null ){
        ...
        if(!empty($category_id)){
            $s .=" AND (categories_id = $category_id ";
            $s .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id = $category_id ) ";
            $s .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id IN(SELECT id FROM product_category WHERE parent_id = $category_id ))) ";
        }
    ...
    function getSqlNewsByCategoryIdByAdmin($category_id){
        ...
        $sql .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id = $category_id AND product_category.status = 1) ";
        $sql .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id IN(SELECT id FROM product_category";
        $sql .=" WHERE parent_id = $category_id AND product_category.status = 1))) ORDER BY service_resort.status ,service_resort.created_at DESC";
        return $sql;
    }
    function getSqlNewsByUserId($user_id, $category_id=""){
        if(!empty($category_id)){
            ...
            $sql .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id = $category_id AND product_category.status = 1) ";
            $sql .=" OR categories_id IN(SELECT id FROM product_category WHERE parent_id IN(SELECT id FROM product_category";
            $sql .=" WHERE parent_id = $category_id AND product_category.status = 1))) ORDER BY service_resort.created_at DESC";
        }
        else{
            $sql = "SELECT service_resort.*, DATE_FORMAT(service_resort.created_at, '%h:%i:%s - %d/%m/%Y') AS ngaytao,";
            $sql .=" member.username AS user_fullname FROM service_resort INNER JOIN member ON service_resort.user_id = member.user_id ";
    ...
    function getNewsByUserId($user_id){
        ...
        $sql .=", service_resort.tag, service_resort.visit, service_resort.type, product_category.title AS category_name";
        $sql .=",DATE_FORMAT(service_resort.created_at, '%d/%m/%Y') AS ngaytao";
        $sql .=" FROM service_resort INNER JOIN product_category ON (service_resort.categories_id = product_category.id)";

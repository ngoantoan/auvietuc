...
class AdvisoryBlogDao{
    ...
    public function AdvisoryBlogDao(){
        global $db;
        $this->db = &$db;
    }
...
//**********************************************/
    ...
    function getAdvisoryBlog($id){
        return $this->db->GetRow("SELECT * FROM advisory_blog WHERE id = ?", $id);
    }

    function getSqlAdmin3($title = NULL, $category_id = NULL) {
        $sql = "SELECT * FROM advisory_blog";
        $sql = " SELECT a.id as id_onoff, a.id, a.title, a.status, a.created_at FROM advisory_blog as a where 1=1 ";
        if ($category_id != 0)
            $sql .= "  AND a.category_id = $category_id  ";
        if ($title != NULL) {
    ...
    function insertproductBE($category_id, $title, $code, $description, $content, $status){
        $created_at = date("Y-m-d H:i:s");
        $sql = "INSERT INTO advisory_blog(category_id, title, code, description, detail, `status`, created_at)";
        $sql .= " VALUES ('".$category_id."','".$title."','".$code."','".$description."','".$content."','".$status."','".$created_at."')";
    ...
    function updateAdvisoryBlogBE($id,$category_id,$title,$code,$description,$content,$status){
        $sql = "UPDATE advisory_blog SET category_id='".$category_id."',title='".$title."',code='".$code."',description='".$description."',detail='".$content."',`status`='".$status."' WHERE id=".$id;
        $ok = $this->db->Execute($sql);
        if($ok)
            return "1";
        else

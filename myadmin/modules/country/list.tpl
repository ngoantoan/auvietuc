...
<h2 class="ic-default">Danh sách quốc gợi ý du học</h2>
...
<script type="text/javascript">
    function ab_afterShowForm(ids) {
        var pr = $('#cate_id').val();
        $('#cate_id').show();
        $.get("index_ajax.php?q=country&r=getCategory", '', function(resp) {
    ...
    function ab_afterShowFormAdd(ids) {
        $.get("index_ajax.php?q=country&r=getCategory&lang=" + $('#lang').val(), '', function(resp) {
            $('#cate_id').replaceWith('<select style="width:300px;" id="cate_id" name="cate_id" class="FormElement"><option value="">-- ROOT --</option></select>');
            $('#cate_id').html(resp);
            $('#cate_id').val();
        });
...
//        data['description'] = CKEDITOR.instances.detail.getData(va['description']);
    ...
    jQuery(document).ready(function () {
        var selectedId;
        jQuery("#list").jqGrid({
            url: 'index_ajax.php?q=country&r=getJsonList&lang=' + $('#lang').val(),
            datatype: 'json',
            mtype: 'POST',
            styleUI: 'Bootstrap',
            ...
            viewrecords : false,
            toppager: true,
            editurl: "index_ajax.php?q=country&r=update&lang=" + $('#lang').val(),
        });
    ...
    function reload() {
        var value =$('#lang').val();
                jQuery("#list").jqGrid('setGridParam', {
            url: "index_ajax.php?q=country&r=getJsonList&lang=" + value,
            editurl: "index_ajax.php?q=country&r=update&lang=" + value
        }).trigger("reloadGrid");
    }
</script>

...
class c_country extends ModuleAdminBase
{
    ...
    function getJsonList() {
        ...
        $sql = fMakeSqlQuery("SELECT id, title, `status` FROM country where 1=1 and status = 1 ");
    ...
    function update() {
        ...
        switch ($oper) {
            case "edit" :
                $sql = "UPDATE country SET `title`='$title',`status`=$status where `id`='" . $id . "'";
                $this->db->Execute($sql);
                break;
            case "add" :
                $sql = "INSERT INTO country (`title`, `status`)
                        VALUES ('" . $title . "', '" . $status . "')";
                ...
                break;
            case "del" :
                $sql = 'delete from country where id in (' . $id . ")";
                $this->db->Execute($sql);
                return 1;
                break;
        }

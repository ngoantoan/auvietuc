...
class c_comments_blog extends ModuleAdminBase
{
    ...
    function getJsonList() {
        ...
        $sql = fMakeSqlQuery("SELECT id, title,`status` FROM news_items where 1=1");
    ...
    function getSubGridJsonList() {
        $id_blog = $_REQUEST['id'];
        $sql = fMakeSqlQuery("SELECT e.id, c.name, c.email, c.phone, c.address, e.contents, e.status, e.created_at FROM comments_blog as e inner join customer as c on c.id = e.id_customer where e.id_blog = $id_blog order by e.created_at desc");
        $this->loadClass("class.jqgrid_json_service.php");
        $jqg = new jqgridJsonService($sql);
        return $jqg->renderJson();
    }
    ...
    function update() {
        ...
        switch ($oper) {
            case "edit" :
                $sql = "UPDATE comments_blog SET `title`='$title',`status`=$status,
				`sort`=$sort,`code`='$code' where `id`='" . $id . "'";

                $this->db->Execute($sql);
                break;
            case "add" :
                $sql = "INSERT INTO comments_blog (`title`,`code`, `status`, `sort`)
                ...
                break;
            case "del" :
                $sql = 'delete from comments_blog where id in (' . $id . ")";
                $this->db->Execute($sql);
                return 1;
                break;
        }
    ...
    function updatesub() {
        ...
        switch ($oper) {
            case "edit" :
                $sql = "UPDATE comments_blog SET `contents`='$contents', `status`=$status where `id`='" . $id . "'";
                $this->db->Execute($sql);
                break;
            case "add" :
                $sql = "INSERT INTO comments_blog (`title`, `code`, `status`, `sort`)

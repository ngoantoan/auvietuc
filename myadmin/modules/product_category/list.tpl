...
<h2 class="ic-default">Danh mục dịch vụ</h2>
...
//        data['description'] = CKEDITOR.instances.detail.getData(va['description']);
    ...
    jQuery(document).ready(function () {
        var selectedId;
        jQuery("#list").jqGrid({
            url: 'index_ajax.php?q=product_category&r=getJsonList&lang=' + $('#lang').val(),
            datatype: 'json',
            mtype: 'POST',
            styleUI: 'Bootstrap',
            colNames: ['Chỉnh sửa', 'Tiêu đề', 'Mã', 'Hiển thị', 'Sắp xếp'],
            colModel: [{
                name : 'id',
                width : 120,
                index : 'id',
            ...
            viewrecords : false,
            toppager: true,
            editurl: "index_ajax.php?q=product_category&r=update&lang=" + $('#lang').val(),
            subGridRowExpanded: function (subgrid_id, row_id) {
                ...
                jQuery("#" + subgrid_table_id).jqGrid({
                    url: "index_ajax.php?q=product_category&r=getSubGridJsonList&lang=" + $('#lang').val()+"&id=" + row_id,
                    datatype: "json",
                    styleUI: 'Bootstrap',
                    colNames: ['ID', 'Tiêu đề', 'Mã', 'Hiển thị', 'Sắp xếp'],
                    colModel: [{
                        name: 'id',
                        index: 'id',
                        key: true,
                    ...
                    multiboxonly: true,
                    viewrecords: true,
                    editurl: "index_ajax.php?q=product_category&r=updatesub&lang=" + $('#lang').val()+ "&parent_id=" + row_id
    ...
    function reload() {
        var value =$('#lang').val();
                jQuery("#list").jqGrid('setGridParam', {
            url: "index_ajax.php?q=product_category&r=getJsonList&lang=" + value,
            editurl: "index_ajax.php?q=product_category&r=update&lang=" + value
        }).trigger("reloadGrid");
    }
</script>

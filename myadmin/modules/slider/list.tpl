...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        jQuery("#list").jqGrid({
            url: 'index_ajax.php?q=slider&r=getJsonList&lang=' + $('#lang').val(),
            datatype: 'json',
            styleUI: 'Bootstrap',
            mtype: 'POST',
            colNames: ['Chỉnh sửa', 'Trạng thái', 'Tiêu đề', 'Link', 'Image', 'kích hoạt', 'Sắp xếp', 'Ngày tạo', 'Ảnh'],
            colModel: [{
                name: 'act',
                width: 100,
                search: false,
            ...
            editurl: "index_ajax.php?q=slider&r=update&lang=" + $("#lang").val(),
        ...
        jQuery("#list").jqGrid('navButtonAdd', '#list_toppager', {
            ...
            onClickButton: function () {
                ...
                var url = 'index_ajax.php?q=slider&r=show&lang=' + $("#lang").val();
    ...
    function edit(_id) {
        $("#img_load_gif").removeClass("hidden");
        if (_id) {
            var url = 'index_ajax.php?q=slider&r=show&lang=' + $("#lang").val()+'&id=' + _id ;
            $('.modal-body').load(url, function (result) {
                $('#myModal').modal({show:true});
                $("#img_load_gif").addClass("hidden");
            });
    ...
    function getList(lang_id) {
        $.post("index_ajax.php?q=slider&r=getCategory3", "", function (res) {
            reload();
        });
    }
    function reload() {
    ...
    function update_status(_id) {
        ...
        var url = 'index_ajax.php?q=slider&r=update&oper=update_status&id=' + _id + '&status=' + status + '&lang=' + $("#lang").val();
    ...
    function duplicate(_id) {
        $("#img_load_gif").removeClass("hidden");
        if (_id) {
            var url = 'index_ajax.php?q=slider&r=show_copy&id=' + _id + '&lang=' + $("#lang").val();
            $('.modal-body').load(url, function (result) {
                $('#myModal').modal({show:true});
                $("#img_load_gif").addClass("hidden");
            });

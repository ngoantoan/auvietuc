...
require_once("../dao/advisory_textDao.php");

class c_advisory_text extends ModuleAdminBase
{
    var $advisory_textDao;

    public function __construct() {
		parent::baseModule();
        $this->advisory_textDao = new advisory_textDao();
    }
    
    function c_advisory_text() {
        parent::baseModule();
        $this->advisory_textDao = new advisory_textDao();
    }
    ...
    function getJsonList() {
        ...
        $jqg = new jqgridJsonService($this->advisory_textDao->getSqlAdmin3($title));
    ...
    function show($msg = "", $reload = 0) {
        ...
        $lang = getRequest('lang');
        $tpl->assign('action', "?q=advisory_text&r=update&lang=" . $lang);
        ...
        if ($id > 0) {//case edit
            $arr = $this->advisory_textDao->getproject($id);
            if ($arr != NULL) {
    ...
    function update()
    {
        ...
        switch ($oper) {
            case "update_status" :
                $bool = $this->db->Execute("UPDATE advisory_text SET status=? WHERE id=?", array($status, $id));
                if ($bool)
                    return nl2br(1);
                else
                    return nl2br(0);
            ...
            case "edit" :
                ...
                break;
            case "editdetail" :
                $bool = $this->advisory_textDao->updateproductBE($id, $title, addslashes($detail), $status);
                if ($bool)
                    return nl2br(1);
                else
                    return nl2br(0);

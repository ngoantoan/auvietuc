...
class c_tag_education extends ModuleAdminBase
{
    ...
    function getJsonList() {
        ...
        $sql = fMakeSqlQuery("SELECT id, title, `status` FROM tag_education where 1=1 and status = 1 ");
    ...
    function update() {
        ...
        switch ($oper) {
            case "edit" :
                $sql = "UPDATE tag_education SET `title`='$title',`status`=$status where `id`='" . $id . "'";
                $this->db->Execute($sql);
                break;
            case "add" :
                $sql = "INSERT INTO tag_education (`title`, `status`)
                        VALUES ('" . $title . "', '" . $status . "')";
                ...
                break;
            case "del" :
                $sql = 'delete from tag_education where id in (' . $id . ")";
                $this->db->Execute($sql);
                return 1;
                break;
        }

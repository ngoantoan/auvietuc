...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery("#list").jqGrid({
            url: "index_ajax.php?q=advisory_blog&r=getJsonList&lang=" + $('#lang').val() + "&category_id=" + $("#selProductCate").val(),
            datatype: 'json',
            styleUI: 'Bootstrap',
            mtype: 'POST',
            ...
            editurl: "index_ajax.php?q=advisory_blog&r=update&lang=" + $("#lang").val(),
        ...
        $('#selProductCate').change(function () {
            ...
            jQuery("#list").jqGrid('setGridParam', {
                url: "index_ajax.php?q=advisory_blog&r=getJsonList&category_id=" + value + "&lang=" + $("#lang").val()
            }).trigger("reloadGrid");
        });
        ...
        jQuery("#list").jqGrid('navButtonAdd', '#list_toppager', {
            ...
            onClickButton: function () {
                ...
                var url = 'index_ajax.php?q=advisory_blog&r=show&lang=' + $("#lang").val() + "&category_id=" + $("#selProductCate").val();
        ...
        //     $.ajax({
        //         url: "index_ajax.php?q=advisory_blog&r=get_category_step2&category_id=" + value + "&lang=" + $("#lang").val(),
    ...
    function update_status(_id) {
            ...
            var url = 'index_ajax.php?q=advisory_blog&r=update&oper=update_status&id=' + _id + '&status=' + status + '&lang=' + $("#lang").val();
    ...
    function edit(_id) {
        $("#img_load_gif").removeClass("hidden");
        if (_id) {
            var url = 'index_ajax.php?q=advisory_blog&r=show&id=' + _id + '&lang=' + $("#lang").val();
            $('.modal-body').load(url, function (result) {
                $('#myModal').modal({show:true});
                $("#img_load_gif").addClass("hidden");
            });
    ...
    function del(_id) {
        if (_id) {
            var url = 'index_ajax.php?q=advisory_blog&r=update&oper=del&id=' + _id + '&lang=' + $("#lang").val();
            $.post( url, function( data ) {
                return true;
            });
        } else {
    ...
    function duplicate(_id) {
        $("#img_load_gif").removeClass("hidden");
        if (_id) {
            var url = 'index_ajax.php?q=advisory_blog&r=show_copy&id=' + _id + '&lang=' + $("#lang").val();
            $('.modal-body').load(url, function (result) {
                $('#myModal').modal({show:true});
                $("#img_load_gif").addClass("hidden");
            });
    ...
    function getList(lang) {
        $.post("index_ajax.php?q=advisory_blog&r=getCategory3", "", function (res) {
